...
ClusterTree ClusterGenerator::getHierarchicalCluster(std::vector<POI>& pointsOfInterests) {
	...
	//bottom up cluster generation
	//generate initial clusters 
	ClusterTree H_Cluster;

	for (int i = 0; i < pointsOfInterests.size(); i++) {
		ClusterTree::Node* newNode = new ClusterTree::Node(pointsOfInterests[i]);
		H_Cluster.addNode(&H_Cluster.root, newNode);
		//std::cout << "Node added with poi.x[" << pointsOfInterests[i].x << "] and poi.y[" << pointsOfInterests[i].y << "]" << std::endl;
	}
	//std::cout << "Tree size ["<< H_Cluster.getSize() <<"]" << std::endl;
	...
	//for (auto node : leafs) {
	//	std::cout << "Node point list:" << std::endl;
	//	for (auto point : node->cluster.points)
	...
	while (leafs.size() > 1) {
		...
		H_Cluster.mergeNodes(minDistanceNodes.node1, minDistanceNodes.node2);
